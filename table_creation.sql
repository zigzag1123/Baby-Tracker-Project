...
CREATE TABLE tbl_child
	(
        ...
        CONSTRAINT c_not_null CHECK(fld_c_id_pk IS NOT NULL AND fld_c_fname IS NOT NULL AND fld_c_lname IS NOT NULL),
        CONSTRAINT c_fk FOREIGN KEY(fld_c_p_username_fk) REFERENCES tbl_parents(fld_p_username_pk)
	);
CREATE TABLE tbl_event_data
	(
		fld_c_id_fk_pk INTEGER,
        fld_event_type VARCHAR(48),
        fld_e_doc TIMESTAMP DEFAULT NOW(),
        CONSTRAINT e_d_fk FOREIGN KEY(fld_c_id_fk_pk) REFERENCES tbl_child(fld_c_id_pk),
        CONSTRAINT e_d_pk PRIMARY KEY(fld_c_id_fk_pk),
        CONSTRAINT e_d_not_null CHECK(fld_c_id_fk_pk IS NOT NULL AND fld_event_type IS NOT NULL AND fld_e_doc IS NOT NULL)
	);
